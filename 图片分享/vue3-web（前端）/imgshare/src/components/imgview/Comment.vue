...
const delClick = (comment:Commentt, user:User) => {
    commentApi.delClick(comment.cid, user.uid).then((value) => {
        const { code, message, date } = value.data
        if (code === 1) {
            ElMessage.success(message)
            getDate()
                ...
                <template #footer>
                    <el-row>
                        ...
                        <el-col :span="4"><svg @click="clicklike(useUserStore().user, item)"
                            ...
                            <svg @click="delClick(item,useUserStore().user)" v-if="clickComment.indexOf(item.cid) != -1" xmlns="http://www.w3.org/2000/svg"
