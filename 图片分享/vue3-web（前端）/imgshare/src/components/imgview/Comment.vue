...
const getClickLike = () => {
    commentApi.getClickLike().then((value) => {
        console.log(value)
...
onMounted(getDate)
const addComment = () => {
    commentApi.addComment(props.img.iid, textarea.value).then((value) => {
        console.log(value);
...
const clicklike = (comment: Commentt) => {
    commentApi.clickLike(comment.cid).then((value) => {
        console.log(value);
        const { code, message, date } = value.data
        if (code === 1) {
            ElMessage.success(message)
...
const delClick = (comment: Commentt) => {
    commentApi.delClick(comment.cid).then((value) => {
        const { code, message, date } = value.data
        if (code === 1) {
            ElMessage.success(message)
            getDate()
        ...
        <el-col v-for="item in comment"><el-card>
                ...
                <template #footer>
                    <el-row>
                        ...
                        <el-col :span="4"><svg @click="clicklike(item)"
                            ...
                            <svg @click="delClick(item)" v-if="clickComment.indexOf(item.cid) != -1"
